...
function capticalTableName(table) {
...
module.exports = (function () {
            ...
            .success(function (tables) {
                ...
                async.each(tables, function (table, _callback) {
                    ...
                    async.each(tableNames, function (table, _callback) {
                        ...
                        text[table] += spaces + "return sequelize.define('" + capticalTableName(table) + "', { \n";

                        fields.forEach(function (field, i) {
                            text[table] += spaces + spaces + capticalField(field) + ": {\n";
                            ...
                            if (Sequelize.Utils._.isString(defaultVal) && defaultVal.toLowerCase().indexOf('nextval') !== -1 && defaultVal.toLowerCase().indexOf('regclass') !== -1) {
                                ...
                                var isUserTable = table === 'user' && field === 'user_id';
                                isUserTable && console.log("field: ", field, ", fieldAttr: ", _tables[table][field]);
    ...
    AutoSequelize.prototype.write = function (attributes, callback) {
        ...
        async.series([
            ...
            async.each(tables, function (table, _callback) {
                fs.writeFile(self.options.directory + '/' + capticalTableName(table) + '.js', attributes[table], function (err) {
